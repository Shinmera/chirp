...
(define-make-* (search-metadata)
  :max-id :since-id :refresh-url :next-results (:count . :result-count) :completed-in :query)
...
According to spec https://dev.twitter.com/docs/api/1.1/get/search/tweets"
  ...
  (let* ((geocode (format NIL "~a,~a,~a" latitude longitude radius))
         (data (signed-request *search/tweets*
                               :parameters (prepare* query geocode locale language result-type count until since-id max-id include-entities)
                               :method :GET)))
